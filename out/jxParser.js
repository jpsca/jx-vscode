...
const RX_IMPORT = /\{#-?\s*import\s+"([^"]+)"\s+as\s+([A-Z][0-9A-Za-z_.:$-]*)\s*-?#\}/g;
function parseImports(document) {
    ...
    while ((match = RX_IMPORT.exec(text)) !== null) {
        ...
        results.push({
            name: nameValue,
            path: pathValue,
            pathRange: new vscode.Range(document.positionAt(pathStart), document.positionAt(pathEnd)),
            nameRange: new vscode.Range(document.positionAt(nameStart), document.positionAt(nameEnd)),
        });
    }
    return results;
}
//# sourceMappingURL=jxParser.js.map