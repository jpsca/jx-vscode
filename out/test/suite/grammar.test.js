...
suite("Grammar", () => {
    ...
    suiteSetup(async () => {
        ...
        const grammarPath = path.resolve(__dirname, "../../../syntaxes/jx.tmLanguage.json");
    ...
    function tokenize(line) {
        ...
        return result.tokens.map((token, i) => {
            const end = i + 1 < result.tokens.length
    ...
    test("Jinja expression in component attribute is highlighted correctly", () => {
        ...
        assert.ok(hasScope(titleAttr, "entity.other.attribute-name"), `title should be attribute name, got: ${titleAttr.scopes.join(", ")}`);
        const openBraces = tokens.find((t) => t.text.includes("{{"));
        assert.ok(openBraces, "Should find {{ token");
        assert.ok(hasScope(openBraces, "punctuation.definition.expression.begin.jinja"), `{{ should be jinja expression begin, got: ${openBraces.scopes.join(", ")}`);
        const pageVar = tokens.find((t) => t.text === "page");
        assert.ok(pageVar, "Should find page token");
        assert.ok(hasScope(pageVar, "variable.other.jinja"), `page should be jinja variable, got: ${pageVar.scopes.join(", ")}`);
        assert.ok(!hasScope(pageVar, "string.unquoted"), `page should NOT be unquoted string, got: ${pageVar.scopes.join(", ")}`);
        const closeBraces = tokens.find((t) => t.text.includes("}}"));
        assert.ok(closeBraces, "Should find }} token");
        assert.ok(hasScope(closeBraces, "punctuation.definition.expression.end.jinja"), `}} should be jinja expression end, got: ${closeBraces.scopes.join(", ")}`);
    });
    test("quoted attribute values still work in component tags", () => {
        ...
        assert.ok(hasScope(attr, "entity.other.attribute-name"), `description should be attribute name, got: ${attr.scopes.join(", ")}`);
        const value = tokens.find((t) => t.text.includes('"Hello"'));
        assert.ok(value, "Should find quoted value token");
        assert.ok(hasScope(value, "string.quoted"), `"Hello" should be quoted string, got: ${value.scopes.join(", ")}`);
    });
    test("mixed quoted and expression attributes", () => {
        const tokens = tokenize('<Layout title={{ page.title }} description="Hello">');
        ...
        assert.ok(hasScope(openBraces, "punctuation.definition.expression.begin.jinja"));
